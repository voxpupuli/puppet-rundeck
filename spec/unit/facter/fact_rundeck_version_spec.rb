...
describe Facter::Util::Fact do
  ...
  context 'rundeck installed | rd-acl in path with current output format' do
    before do
      allow(Facter::Util::Resolution).to receive('which').with('rd-acl').and_return(true)
      allow(Facter::Util::Resolution).to receive('exec').with('rd-acl -h').and_return('[RUNDECK version 3.0.6-20180917 (0)]')
    end
  ...
  context 'rundeck installed | rd-acl in path with old output format' do
    before do
      allow(Facter::Util::Resolution).to receive('which').with('rd-acl').and_return(true)
      allow(Facter::Util::Resolution).to receive('exec').with('rd-acl -h').and_return('[RUNDECK version 2.0.0 (0)]')
    end
